...
State vector of a quantum state is expressed as {class}`scaluq.StateVector`.
...
{class}`scaluq.StateVector` is constructed with single integer value `n_qubits`, which is the number of qubits.
Optional `precision` (`'f64'` by default) and `space` (`'default'` by default) keyword arguments select floating-point precision and execution space.
...
{func}`n_qubits <scaluq.StateVector>` returns the number of qubits $n$.

{func}`dim <scaluq.StateVector>` returns the dimension of the vector $2^n$.

{func}`get_amplitudes <scaluq.StateVector>` returns the content of StateVector as type `list[complex]`.
...
{func}`Haar_random_state <scaluq.StateVector>` initialize state with Haar random state. You pass seed as an optional argument. Without passing seed, the random device of system is used.

{func}`uninitialized_state <scaluq.StateVector>` only allocate memory on the execution space without initializing. The content of the vector is undefined. It is not guaranteed to be normalized. Since allocating the vector by this function is faster than other initializing functions, you should use this if you will load other vector immediately after.
...
You can load the content of {class}`StateVector <scaluq.StateVector>` by various functions.

{func}`set_zero_state <scaluq.StateVector.set_zero_state>` is used for setting the vector to $\ket{00\dots0}=[1,0,\dots,0]$.

{func}`set_zero_norm_state <scaluq.StateVector.set_zero_norm_state>` is used for setting the vector to $0=[0,0,\dots,0]$.

{func}`set_computational_basis <scaluq.StateVector.set_computational_basis>` is used for setting the vector to $\ket{b}$ with input args $0\leq b \leq 2^{n}-1$.

{func}`load <scaluq.StateVector.load>` is used for loading the other vector (with $2^n$ length) directly as amplitudes.
...
When you work with {class}`StateVectorBatched <scaluq.StateVectorBatched>`, you can extract a single state vector in two ways.

- {func}`get_state_vector_at <scaluq.StateVectorBatched.get_state_vector_at>` returns a copy.
- {func}`view_state_vector_at <scaluq.StateVectorBatched.view_state_vector_at>` returns a view (shared memory).

`view_state_vector_at` does not copy the amplitudes, so updating the returned {class}`StateVector <scaluq.StateVector>` also updates the corresponding batch in the original `StateVectorBatched`.
...
You can perform some operations to {class}`StateVector <scaluq.StateVector>`.

`add_state_vector_with_coef` is used to update the vector by adding $c\ket{\psi}$, where `c` is a complex number and $\ket{\psi}$ is another state vector with same dimension.

`multiply_coef` is used to update the vector by multiplying a complex number.
...
You can get probabilistic measures of {class}`StateVector <scaluq.StateVector>`.

{func}`get_zero_probability <scaluq.StateVector.get_zero_probability>` is used to get the probability of getting $0$ when the specified qubit is measured by Z-basis.

{func}`get_marginal_probability <scaluq.StateVector.get_marginal_probability>` is used to get the marginal probability of getting specified result when some of qubits are measured simultaneously by Z-basis.
...
- {func}`StateVector.UNMEASURED <scaluq.StateVector.UNMEASURED>`: $i$-th qubit is not measured.

{func}`get_entropy <scaluq.StateVector.get_entropy>` is used to get the entropy of the vector, which is calculated by $\sum_i -p_i \log_2 p_i$ ($p_i$ ($0\leq i<2^n$) is $|v_i|^2$ with $v_i$ is the $i$-th amplitude of the vector).

{func}`sampling <scaluq.StateVector.sampling>` is used to perform sampling on the vector.
