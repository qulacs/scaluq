...
量子回路は {class}`scaluq.Circuit` で表されています。
量子回路は作用させる {class}`scaluq.Gate` または {class}`scaluq.ParamGate` のインスタンス配列をもちます。
...
## 回路にゲートを追加する
{func}`add_gate <scaluq.Circuit.add_gate>` により、`Circuit`に`Gate`を追加できます。`Gate`は浅いコピーで追加されます。Scaluqのすべての`Gate`はイミュータブルであり、これは常に安全です。
...
{class}`scaluq.StateVector`に{func}`update_quantum_state <scaluq.Circuit.update_quantum_state>`を適用することにより、回路を作用させることができます。
