...
Scaluqにおいて、量子演算子は{class}`scaluq.Operator`で表されます。
これは1つ以上の{class}`scaluq.PauliOperator`インスタンスにより構成されます。

## PauliOperator

{class}`scaluq.PauliOperator`は、特定の量子ビットに作用するパウリ演算子 (I,X,Y,Z)のテンソル積に、複素係数 (coef)を乗じたものを表します。
...
# IDs: 0=I, 1=X, 2=Y, 3=Z
p2 = PauliOperator.from_targets_and_pauli_ids(target_qubit_list=[0, 1, 2], pauli_id_list=[1, 0, 2], coef=1.0)

# 3. ビットマスクにより初期化
# bit_flip_mask: Xの位置, phase_flip_mask: Zの位置
p3 = PauliOperator.from_XZ_mask(bit_flip_mask=0b101, phase_flip_mask=0b010, coef=1.0)
```

## Operator

{class}`scaluq.Operator`は複数の{class}`scaluq.PauliOperator`の項の和で定義される一般的な演算子 (ハミルトニアンなど)を表します。
...
定義した演算子を用いて、{class}`scaluq.StateVector`から物理的な情報を取得できます。
