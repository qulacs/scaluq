...
|`SCALUQ_USE_HIP`|`OFF`|`実験的機能` GPU (HIP)での並列処理を行うか|
...
|`SCALUQ_HIP_ARCH`|-|`SCALUQ_USE_HIP=ON`の場合、ターゲットとなるAMD GPU アーキテクチャ (名前は[Kokkos CMake Keywords](https://kokkos.org/kokkos-core-wiki/get-started/configuration-guide.html)を参照、例: `SCALUQ_HIP_ARCH=AMD_GPU`)|
