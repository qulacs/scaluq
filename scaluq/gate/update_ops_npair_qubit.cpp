...
void fusedswap_gate(UINT target_qubit_index_0,
    ...
    Kokkos::parallel_for(
        Kokkos::MDRangePolicy<Kokkos::Rank<2>>({0, 0}, {states.batch_size(), n_qubits}),
        KOKKOS_LAMBDA(const UINT batch_id, const UINT i) {
            const UINT kblk = (i & kblk_mask) >> upper_index;
            const UINT jblk = (i & jblk_mask) >> lower_index;
            if (jblk > kblk) {
                const UINT j = (i & else_mask) | jblk << upper_index | kblk << lower_index;
                Kokkos::Experimental::swap(states._raw(batch_id, i), states._raw(batch_id, j));
            }
        });
    Kokkos::fence();
}
