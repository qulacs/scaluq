...
public:
    ...
    void apply_to_state(StateVector<FloatType>& state_vector) const {
        ...
        for (const auto& term : _terms) {
            StateVector<FloatType> tmp = state_vector.copy();
    ...
    [[nodiscard]] Complex get_expectation_value(const StateVector<FloatType>& state_vector) const {
        ...
        Kokkos::
            View<const PauliOperator*, Kokkos::HostSpace, Kokkos::MemoryTraits<Kokkos::Unmanaged>>
    ...
    template <std::floating_point FloatType>
    [[nodiscard]] Complex get_expectation_value_loop(
        const StateVector<FloatType>& state_vector) const;
