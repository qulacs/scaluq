...
void apply_pauli(std::uint64_t control_mask,
    ...
    for (std::uint64_t b = 0; b < rsize; b += batch_size)
        Kokkos::parallel_for(
            "apply_pauli",
            Kokkos::TeamPolicy<Sp>(Sp(), std::min(batch_size, rsize - b), Kokkos::AUTO),
            KOKKOS_LAMBDA(const Kokkos::TeamPolicy<Sp>::member_type& team) {
                std::uint64_t batch_id = team.league_rank() + b;
                Kokkos::parallel_for(
                    Kokkos::TeamThreadRange(team, dim >> (Kokkos::popcount(control_mask) + 1)),
                    [&](const std::uint64_t i) {
...
void apply_pauli_rotation(std::uint64_t control_mask,
    ...
    const Complex<State::prec> cosval = internal::cos(-half_angle);
    const Complex<State::prec> sinval = internal::sin(-half_angle);
    if (bit_flip_mask == 0) {
        const Complex<State::prec> cval_min = cosval - Complex<State::prec>(0, 1) * sinval;
        const Complex<State::prec> cval_pls = cosval + Complex<State::prec>(0, 1) * sinval;
        Kokkos::parallel_for(
            "apply_pauli_rotation",
            Kokkos::RangePolicy<SpaceType<State::space>>(0,
                                                         flat_dim >> std::popcount(control_mask)),
            KOKKOS_LAMBDA(std::uint64_t i) {
                std::uint64_t state_idx =
                    insert_zero_at_mask_positions(i, control_mask) | control_value_mask;
                if (Kokkos::popcount(state_idx & phase_flip_mask) & 1) {
    ...
    Kokkos::parallel_for(
        "apply_pauli_rotation",
        Kokkos::RangePolicy<SpaceType<State::space>>(0,
                                                     flat_dim >> (std::popcount(control_mask) + 1)),
        KOKKOS_LAMBDA(std::uint64_t i) {
            std::uint64_t basis_0 =
                insert_zero_at_mask_positions(i, control_mask | 1ULL << pivot) | control_value_mask;
            std::uint64_t basis_1 = basis_0 ^ bit_flip_mask;
