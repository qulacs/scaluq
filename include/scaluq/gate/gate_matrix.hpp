...
class OneTargetDenseMatrixGateImpl : public GateBase<Prec> {
    Matrix2x2<Prec> _matrix;
    bool _is_unitary;

public:
    OneTargetDenseMatrixGateImpl(std::uint64_t target_mask,
...
template <Precision Prec, ExecutionSpace Space>
class TwoTargetDenseMatrixGateImpl : public GateBase<Prec> {
    Matrix4x4<Prec> _matrix;
    bool _is_unitary;

public:
    TwoTargetDenseMatrixGateImpl(std::uint64_t target_mask,
                                 std::uint64_t control_mask,
                                 std::uint64_t control_value_mask,
                                 const ComplexMatrix& mat,
                                 bool is_unitary = false);
...
template <Precision Prec, ExecutionSpace Space>
using OneTargetDenseMatrixGate =
    internal::GatePtr<internal::OneTargetDenseMatrixGateImpl<Prec, Space>>;
template <Precision Prec, ExecutionSpace Space>
using TwoTargetDenseMatrixGate =
    internal::GatePtr<internal::TwoTargetDenseMatrixGateImpl<Prec, Space>>;
...
void bind_gate_gate_matrix_hpp(nb::module_& m, nb::class_<Gate<Prec>>& gate_base_def) {
    ...
    bind_specific_gate<OneTargetDenseMatrixGate<Prec, Space>, Prec>(
        m,
        gate_base_def,
        "OneTargetDenseMatrixGate",
        "Specific class of one-target dense matrix gate.");
    bind_specific_gate<TwoTargetDenseMatrixGate<Prec, Space>, Prec>(
        m,
        gate_base_def,
        "TwoTargetDenseMatrixGate",
        "Specific class of two-target dense matrix gate.");
}
}  // namespace internal
#endif
}  // namespace scaluq
