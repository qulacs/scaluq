...
struct complex {
    ...
    template <class U>
    complex(const complex<U>& o) : _r(T(o._r)), _i(T(o._i)) {}
...
public:
    ...
    View(ViewAllocateWithoutInitializing, Layout) {}
    ...
    value_type& operator()(size_type, size_type) const {
        static value_type v{};
        return v;
    }
    value_type& operator[](size_type) const {
...
struct TeamThreadRange {
    template <class M>
    TeamThreadRange(M&&, std::size_t) {}
    template <class M>
    TeamThreadRange(M&&, std::size_t, std::size_t) {}
};
struct ThreadVectorRange {
    template <class M>
    ThreadVectorRange(M&&, std::size_t) {}
    template <class M>
    ThreadVectorRange(M&&, std::size_t, std::size_t) {}
};
template <class... Props>
struct TeamThreadMDRange {
    template <class M, class... Args>
    TeamThreadMDRange(M&&, Args...) {}
};
struct PerTeam {
    template <class M>
    explicit PerTeam(M&&) {}
...
template <class V>
inline auto create_mirror_view(const V& v) {
    return v;
}

template <class Space, class V>
inline auto create_mirror_view(Space, const V& v) {
    return v;
}

template <class Space, class V>
inline auto create_mirror_view_and_copy(Space, const V& v) {
    return v;
}
...
// ── Atomic / bit ops ──────────────────────────────────────────────────────
template <class T>
inline void atomic_add(T* p, T v) {
    *p += v;
}

template <class T>
inline void kokkos_swap(T& a, T& b) {
    T t = a;
    a = b;
    b = t;
}
