...
#define FUNC_MACRO(T, Space) \
    template Kokkos::View<T*, Space> convert_vector_to_view(const std::vector<T>&);
...
#define FUNC_MACRO(T, Space) \
    template std::vector<T> convert_view_to_vector(const Kokkos::View<T*, Space>&);
SCALUQ_CALL_MACRO_FOR_TYPES_AND_EXECUTION_SPACE(FUNC_MACRO)
#undef FUNC_MACRO
...
inline Matrix<Prec, Space> convert_external_matrix_to_internal_matrix(
    ...
    Kokkos::parallel_for(
        Kokkos::MDRangePolicy<Kokkos::Rank<2>, Space>({0, 0}, {rows, cols}),
...
#define FUNC_MACRO(Prec, Space) \
    template Matrix<Prec, Space> convert_external_matrix_to_internal_matrix(const ComplexMatrix&);
...
#define FUNC_MACRO(Prec, Space) \
    template ComplexMatrix convert_internal_matrix_to_external_matrix(const Matrix<Prec, Space>&);
SCALUQ_CALL_MACRO_FOR_PRECISION_AND_EXECUTION_SPACE(FUNC_MACRO)
#undef FUNC_MACRO
...
#define FUNC_MACRO(Prec, Space) \
    template ComplexMatrix convert_coo_to_external_matrix(SparseMatrix<Prec, Space>);
SCALUQ_CALL_MACRO_FOR_PRECISION_AND_EXECUTION_SPACE(FUNC_MACRO)
#undef FUNC_MACRO
