...
void two_target_dense_matrix_gate_simd(std::uint64_t target_mask,
    ...
    Kokkos::parallel_for(
        ...
        KOKKOS_LAMBDA(std::uint64_t g) {
            ...
            (m01.fma(v1, m00 * v0) + m03.fma(v3, m02 * v2)).store_aligned(&state.at_unsafe(basis0));
            (m11.fma(v1, m10 * v0) + m13.fma(v3, m12 * v2)).store_aligned(&state.at_unsafe(basis1));
            (m21.fma(v1, m20 * v0) + m23.fma(v3, m22 * v2)).store_aligned(&state.at_unsafe(basis2));
            (m31.fma(v1, m30 * v0) + m33.fma(v3, m32 * v2)).store_aligned(&state.at_unsafe(basis3));
        });
}
...
void two_target_dense_matrix_gate_simd_middle(std::uint64_t target_mask,
    ...
    using C02 = SimdCoefFor<SimdType, common_coef_kind(M02, M13)>;
    using C03 = SimdCoefFor<SimdType, common_coef_kind(M03, M12)>;
    using C20 = SimdCoefFor<SimdType, common_coef_kind(M20, M31)>;
    using C21 = SimdCoefFor<SimdType, common_coef_kind(M21, M30)>;
    using C22 = SimdCoefFor<SimdType, common_coef_kind(M22, M33)>;
    using C23 = SimdCoefFor<SimdType, common_coef_kind(M23, M32)>;
...
void two_target_dense_matrix_gate_simd_low(std::uint64_t control_mask,
                                           ...
                                           State& state) {
    using SimdType = SimdComplex<State::prec>;
    using C0 = SimdCoefFor<SimdType, common_coef_kind(M00, M11, M22, M33)>;
    using C1 = SimdCoefFor<SimdType, common_coef_kind(M01, M10, M23, M32)>;
    using C2 = SimdCoefFor<SimdType, common_coef_kind(M02, M13, M20, M31)>;
    using C3 = SimdCoefFor<SimdType, common_coef_kind(M03, M12, M21, M30)>;

    constexpr std::size_t complex_lanes = SimdType::complex_lanes;
    static_assert(TargetBit0 < TargetBit1);
    static_assert((1ULL << TargetBit1) < complex_lanes);

    const auto m00 = C0::template select_complex_lane_bits<TargetBit0, TargetBit1>(
        matrix[0][0], matrix[1][1], matrix[2][2], matrix[3][3]);
    const auto m01 = C1::template select_complex_lane_bits<TargetBit0, TargetBit1>(
        matrix[0][1], matrix[1][0], matrix[2][3], matrix[3][2]);
    const auto m02 = C2::template select_complex_lane_bits<TargetBit0, TargetBit1>(
        matrix[0][2], matrix[1][3], matrix[2][0], matrix[3][1]);
    const auto m03 = C3::template select_complex_lane_bits<TargetBit0, TargetBit1>(
    ...
    Kokkos::parallel_for(
        ...
        KOKKOS_LAMBDA(std::uint64_t g) {
            ...
            const auto v3 = v0.template permute_complex_lanes_xor<(1ULL << TargetBit1) |
                                                                  (1ULL << TargetBit0)>();
            (m01.fma(v1, m00 * v0) + m03.fma(v3, m02 * v2)).store_aligned(&state.at_unsafe(basis));
        });
}
