...
double run(UINT n_q, UINT batch, UINT loop = 10) {
...
double run_serial(UINT n_q, UINT batch, UINT loop = 10) {
    std::cout << "n_qubits: " << n_q << ", batch: " << batch << std::endl;
    Kokkos::Timer tm;
    tm.reset();
    for (UINT i = 0; i < loop; ++i) {
        std::vector<StateVector> states(batch, StateVector::Haar_random_state(n_q, 0));
