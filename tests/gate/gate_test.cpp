...
TYPED_TEST(GateTest, ApplyI) {
    ...
    run_random_gate_apply<Prec, Space, gate::I<Prec>>(5);
}
TYPED_TEST(GateTest, ApplyGlobalPhase) {
    constexpr Precision Prec = TestFixture::Prec;
    constexpr ExecutionSpace Space = TestFixture::Space;
    run_random_gate_apply<Prec, Space, gate::GlobalPhase<Prec>>(5);
}
TYPED_TEST(GateTest, ApplyX) {
    constexpr Precision Prec = TestFixture::Prec;
    constexpr ExecutionSpace Space = TestFixture::Space;
    run_random_gate_apply<Prec, Space, gate::X<Prec>>(5, make_X);
}
TYPED_TEST(GateTest, ApplyY) {
    constexpr Precision Prec = TestFixture::Prec;
    constexpr ExecutionSpace Space = TestFixture::Space;
    run_random_gate_apply<Prec, Space, gate::Y<Prec>>(5, make_Y);
}
TYPED_TEST(GateTest, ApplyZ) {
    constexpr Precision Prec = TestFixture::Prec;
    constexpr ExecutionSpace Space = TestFixture::Space;
    run_random_gate_apply<Prec, Space, gate::Z<Prec>>(5, make_Z);
}
TYPED_TEST(GateTest, ApplyH) {
    constexpr Precision Prec = TestFixture::Prec;
    constexpr ExecutionSpace Space = TestFixture::Space;
    run_random_gate_apply<Prec, Space, gate::H<Prec>>(5, make_H);
}
TYPED_TEST(GateTest, ApplyS) {
    constexpr Precision Prec = TestFixture::Prec;
    constexpr ExecutionSpace Space = TestFixture::Space;
    run_random_gate_apply<Prec, Space, gate::S<Prec>>(5, make_S);
}
TYPED_TEST(GateTest, ApplySdag) {
    constexpr Precision Prec = TestFixture::Prec;
    constexpr ExecutionSpace Space = TestFixture::Space;
    run_random_gate_apply<Prec, Space, gate::Sdag<Prec>>(5, make_Sdag);
}
TYPED_TEST(GateTest, ApplyT) {
    constexpr Precision Prec = TestFixture::Prec;
    constexpr ExecutionSpace Space = TestFixture::Space;
    run_random_gate_apply<Prec, Space, gate::T<Prec>>(5, make_T);
}
TYPED_TEST(GateTest, ApplyTdag) {
    constexpr Precision Prec = TestFixture::Prec;
    constexpr ExecutionSpace Space = TestFixture::Space;
    run_random_gate_apply<Prec, Space, gate::Tdag<Prec>>(5, make_Tdag);
}
TYPED_TEST(GateTest, ApplySqrtX) {
    constexpr Precision Prec = TestFixture::Prec;
    constexpr ExecutionSpace Space = TestFixture::Space;
    run_random_gate_apply<Prec, Space, gate::SqrtX<Prec>>(5, make_SqrtX);
}
TYPED_TEST(GateTest, ApplySqrtY) {
    constexpr Precision Prec = TestFixture::Prec;
    constexpr ExecutionSpace Space = TestFixture::Space;
    run_random_gate_apply<Prec, Space, gate::SqrtY<Prec>>(5, make_SqrtY);
}
TYPED_TEST(GateTest, ApplySqrtXdag) {
    constexpr Precision Prec = TestFixture::Prec;
    constexpr ExecutionSpace Space = TestFixture::Space;
    run_random_gate_apply<Prec, Space, gate::SqrtXdag<Prec>>(5, make_SqrtXdag);
}
TYPED_TEST(GateTest, ApplySqrtYdag) {
    constexpr Precision Prec = TestFixture::Prec;
    constexpr ExecutionSpace Space = TestFixture::Space;
    run_random_gate_apply<Prec, Space, gate::SqrtYdag<Prec>>(5, make_SqrtYdag);
}
TYPED_TEST(GateTest, ApplyP0) {
    constexpr Precision Prec = TestFixture::Prec;
    constexpr ExecutionSpace Space = TestFixture::Space;
    run_random_gate_apply<Prec, Space, gate::P0<Prec>>(5, make_P0);
}
TYPED_TEST(GateTest, ApplyP1) {
    constexpr Precision Prec = TestFixture::Prec;
    constexpr ExecutionSpace Space = TestFixture::Space;
    run_random_gate_apply<Prec, Space, gate::P1<Prec>>(5, make_P1);
}
TYPED_TEST(GateTest, ApplyRX) {
    constexpr Precision Prec = TestFixture::Prec;
    constexpr ExecutionSpace Space = TestFixture::Space;
    run_random_gate_apply<Prec, Space, gate::RX<Prec>>(5, make_RX);
}
TYPED_TEST(GateTest, ApplyRY) {
    constexpr Precision Prec = TestFixture::Prec;
    constexpr ExecutionSpace Space = TestFixture::Space;
    run_random_gate_apply<Prec, Space, gate::RY<Prec>>(5, make_RY);
}
TYPED_TEST(GateTest, ApplyRZ) {
    constexpr Precision Prec = TestFixture::Prec;
    constexpr ExecutionSpace Space = TestFixture::Space;
    run_random_gate_apply<Prec, Space, gate::RZ<Prec>>(5, make_RZ);
}
...
void test_standard_gate_control(Factory factory, std::uint64_t n) {
    ...
    if constexpr (num_target == 0 && num_rotation == 1) {
        ...
        test_gate<Prec, Space>(g1, g2, n, control_mask, control_value_mask);
...
void test_pauli_control(std::uint64_t n) {
    ...
    if constexpr (!rotation) {
        Gate<Prec> g1 = gate::Pauli(pauli1, controls, control_values);
        Gate<Prec> g2 = gate::Pauli(pauli2, {});
        test_gate<Prec, Space>(g1, g2, n, control_mask, control_value_mask);
    } else {
        double angle = random.uniform() * std::numbers::pi * 2;
        Gate<Prec> g1 = gate::PauliRotation(pauli1, angle, controls, control_values);
        Gate<Prec> g2 = gate::PauliRotation(pauli2, angle, {});
        test_gate<Prec, Space>(g1, g2, n, control_mask, control_value_mask);
    }
}
...
TYPED_TEST(GateTest, Control) {
    ...
    for ([[maybe_unused]] std::uint64_t _ : std::views::iota(0, 10)) {
        test_standard_gate_control<Prec, Space, 0, 1>(gate::GlobalPhase<Prec>, n);
        test_standard_gate_control<Prec, Space, 1, 0>(gate::X<Prec>, n);
        test_standard_gate_control<Prec, Space, 1, 0>(gate::Y<Prec>, n);
        test_standard_gate_control<Prec, Space, 1, 0>(gate::Z<Prec>, n);
        test_standard_gate_control<Prec, Space, 1, 0>(gate::S<Prec>, n);
        test_standard_gate_control<Prec, Space, 1, 0>(gate::Sdag<Prec>, n);
        test_standard_gate_control<Prec, Space, 1, 0>(gate::T<Prec>, n);
        test_standard_gate_control<Prec, Space, 1, 0>(gate::Tdag<Prec>, n);
        test_standard_gate_control<Prec, Space, 1, 0>(gate::SqrtX<Prec>, n);
        test_standard_gate_control<Prec, Space, 1, 0>(gate::SqrtXdag<Prec>, n);
        test_standard_gate_control<Prec, Space, 1, 0>(gate::SqrtY<Prec>, n);
        test_standard_gate_control<Prec, Space, 1, 0>(gate::SqrtYdag<Prec>, n);
        test_standard_gate_control<Prec, Space, 1, 0>(gate::P0<Prec>, n);
        test_standard_gate_control<Prec, Space, 1, 0>(gate::P1<Prec>, n);
        test_standard_gate_control<Prec, Space, 1, 1>(gate::RX<Prec>, n);
        test_standard_gate_control<Prec, Space, 1, 1>(gate::RY<Prec>, n);
        test_standard_gate_control<Prec, Space, 1, 1>(gate::RZ<Prec>, n);
        test_standard_gate_control<Prec, Space, 1, 1>(gate::U1<Prec>, n);
