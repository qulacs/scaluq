...
void run_random_gate_apply(std::uint64_t n_qubits) {
    ...
    for (int repeat = 0; repeat < 10; repeat++) {
        auto state = StateVector<Prec>::Haar_random_state(n_qubits);
...
void run_random_gate_apply_none_dense(std::uint64_t n_qubits) {
    ...
    for (std::uint64_t rep = 0; rep < max_repeat; rep++) {
        StateVector state = StateVector<Prec>::Haar_random_state(n_qubits);
        auto state_cp = state.get_amplitudes();
        ComplexVector test_state = ComplexVector::Zero(dim);
...
void run_random_gate_apply_general_dense(std::uint64_t n_qubits) {
    ...
    {
        for (std::uint64_t rep = 0; rep < max_repeat; rep++) {
            StateVector<Prec> state = StateVector<Prec>::Haar_random_state(n_qubits);
            auto state_cp = state.get_amplitudes();
            for (std::uint64_t i = 0; i < dim; i++) {
                test_state[i] = state_cp[i];
            }
...
void run_random_gate_apply_pauli(std::uint64_t n_qubits) {
    ...
    for (int repeat = 0; repeat < 10; repeat++) {
        ...
        auto pauli_gate = gate::Pauli<Prec>(pauli);
...
TYPED_TEST(GateTest, ApplyI) {
    ...
    run_random_gate_apply<Prec, gate::I<Prec>>(5);
}
TYPED_TEST(GateTest, ApplyGlobalPhase) {
    constexpr Precision Prec = TestFixture::Prec;
    run_random_gate_apply<Prec, gate::GlobalPhase<Prec>>(5);
}
TYPED_TEST(GateTest, ApplyX) {
    constexpr Precision Prec = TestFixture::Prec;
    run_random_gate_apply<Prec, gate::X<Prec>>(5, make_X);
}
TYPED_TEST(GateTest, ApplyY) {
    constexpr Precision Prec = TestFixture::Prec;
    run_random_gate_apply<Prec, gate::Y<Prec>>(5, make_Y);
}
TYPED_TEST(GateTest, ApplyZ) {
    constexpr Precision Prec = TestFixture::Prec;
    run_random_gate_apply<Prec, gate::Z<Prec>>(5, make_Z);
}
TYPED_TEST(GateTest, ApplyH) {
    constexpr Precision Prec = TestFixture::Prec;
    run_random_gate_apply<Prec, gate::H<Prec>>(5, make_H);
}
TYPED_TEST(GateTest, ApplyS) {
    constexpr Precision Prec = TestFixture::Prec;
    run_random_gate_apply<Prec, gate::S<Prec>>(5, make_S);
}
TYPED_TEST(GateTest, ApplySdag) {
    constexpr Precision Prec = TestFixture::Prec;
    run_random_gate_apply<Prec, gate::Sdag<Prec>>(5, make_Sdag);
}
TYPED_TEST(GateTest, ApplyT) {
    constexpr Precision Prec = TestFixture::Prec;
    run_random_gate_apply<Prec, gate::T<Prec>>(5, make_T);
}
TYPED_TEST(GateTest, ApplyTdag) {
    constexpr Precision Prec = TestFixture::Prec;
    run_random_gate_apply<Prec, gate::Tdag<Prec>>(5, make_Tdag);
}
TYPED_TEST(GateTest, ApplySqrtX) {
    constexpr Precision Prec = TestFixture::Prec;
    run_random_gate_apply<Prec, gate::SqrtX<Prec>>(5, make_SqrtX);
}
TYPED_TEST(GateTest, ApplySqrtY) {
    constexpr Precision Prec = TestFixture::Prec;
    run_random_gate_apply<Prec, gate::SqrtY<Prec>>(5, make_SqrtY);
}
TYPED_TEST(GateTest, ApplySqrtXdag) {
    constexpr Precision Prec = TestFixture::Prec;
    run_random_gate_apply<Prec, gate::SqrtXdag<Prec>>(5, make_SqrtXdag);
}
TYPED_TEST(GateTest, ApplySqrtYdag) {
    constexpr Precision Prec = TestFixture::Prec;
    run_random_gate_apply<Prec, gate::SqrtYdag<Prec>>(5, make_SqrtYdag);
}
TYPED_TEST(GateTest, ApplyP0) {
    constexpr Precision Prec = TestFixture::Prec;
    run_random_gate_apply<Prec, gate::P0<Prec>>(5, make_P0);
}
TYPED_TEST(GateTest, ApplyP1) {
    constexpr Precision Prec = TestFixture::Prec;
    run_random_gate_apply<Prec, gate::P1<Prec>>(5, make_P1);
}
TYPED_TEST(GateTest, ApplyRX) {
    constexpr Precision Prec = TestFixture::Prec;
    run_random_gate_apply<Prec, gate::RX<Prec>>(5, make_RX);
}
TYPED_TEST(GateTest, ApplyRY) {
    constexpr Precision Prec = TestFixture::Prec;
    run_random_gate_apply<Prec, gate::RY<Prec>>(5, make_RY);
}
TYPED_TEST(GateTest, ApplyRZ) {
    constexpr Precision Prec = TestFixture::Prec;
    run_random_gate_apply<Prec, gate::RZ<Prec>>(5, make_RZ);
}

TYPED_TEST(GateTest, ApplyIBMQ) {
    constexpr Precision Prec = TestFixture::Prec;
    run_random_gate_apply_IBMQ<Prec>(5, make_U);
}

TYPED_TEST(GateTest, ApplySparseMatrixGate) {
    constexpr Precision Prec = TestFixture::Prec;
    run_random_gate_apply_sparse<Prec>(6);
}
TYPED_TEST(GateTest, ApplyDenseMatrixGate) {
    constexpr Precision Prec = TestFixture::Prec;
    run_random_gate_apply_none_dense<Prec>(6);
    run_random_gate_apply_single_dense<Prec>(6);
    run_random_gate_apply_general_dense<Prec>(6);
}

TYPED_TEST(GateTest, ApplyPauliGate) {
    constexpr Precision Prec = TestFixture::Prec;
    run_random_gate_apply_pauli<Prec>(5);
}
...
void test_gate(Gate<Prec> gate_control,
    ...
    StateVector<Prec> state_controlled(n_qubits - std::popcount(control_mask));
...
void test_pauli_control(std::uint64_t n) {
    ...
    if constexpr (!rotation) {
        Gate<Prec> g1 = gate::Pauli(PauliOperator<Prec>(data1), controls);
        Gate<Prec> g2 = gate::Pauli(PauliOperator<Prec>(data2), {});
        test_gate(g1, g2, n, control_mask);
    } else {
        double angle = random.uniform() * std::numbers::pi * 2;
        Gate<Prec> g1 = gate::PauliRotation(PauliOperator<Prec>(data1), angle, controls);
        Gate<Prec> g2 = gate::PauliRotation(PauliOperator<Prec>(data2), angle, {});
        test_gate(g1, g2, n, control_mask);
    }
}
...
void test_matrix_control(std::uint64_t n_qubits) {
    ...
    if constexpr (num_target == 0) {
        ...
        test_gate<Prec>(d1, d2, n_qubits, control_mask);
        test_gate<Prec>(s1, s2, n_qubits, control_mask);
...
TYPED_TEST(GateTest, Control) {
    ...
    for ([[maybe_unused]] std::uint64_t _ : std::views::iota(0, 10)) {
        test_standard_gate_control<Prec, 0, 1>(gate::GlobalPhase<Prec>, n);
        test_standard_gate_control<Prec, 1, 0>(gate::X<Prec>, n);
        test_standard_gate_control<Prec, 1, 0>(gate::Y<Prec>, n);
        test_standard_gate_control<Prec, 1, 0>(gate::Z<Prec>, n);
        test_standard_gate_control<Prec, 1, 0>(gate::S<Prec>, n);
        test_standard_gate_control<Prec, 1, 0>(gate::Sdag<Prec>, n);
        test_standard_gate_control<Prec, 1, 0>(gate::T<Prec>, n);
        test_standard_gate_control<Prec, 1, 0>(gate::Tdag<Prec>, n);
        test_standard_gate_control<Prec, 1, 0>(gate::SqrtX<Prec>, n);
        test_standard_gate_control<Prec, 1, 0>(gate::SqrtXdag<Prec>, n);
        test_standard_gate_control<Prec, 1, 0>(gate::SqrtY<Prec>, n);
        test_standard_gate_control<Prec, 1, 0>(gate::SqrtYdag<Prec>, n);
        test_standard_gate_control<Prec, 1, 0>(gate::P0<Prec>, n);
        test_standard_gate_control<Prec, 1, 0>(gate::P1<Prec>, n);
        test_standard_gate_control<Prec, 1, 1>(gate::RX<Prec>, n);
        test_standard_gate_control<Prec, 1, 1>(gate::RY<Prec>, n);
        test_standard_gate_control<Prec, 1, 1>(gate::RZ<Prec>, n);
        test_standard_gate_control<Prec, 1, 1>(gate::U1<Prec>, n);
        test_standard_gate_control<Prec, 1, 2>(gate::U2<Prec>, n);
        test_standard_gate_control<Prec, 1, 3>(gate::U3<Prec>, n);
        test_standard_gate_control<Prec, 2, 0>(gate::Swap<Prec>, n);
        test_pauli_control<Prec, false>(n);
        test_pauli_control<Prec, true>(n);
        test_matrix_control<Prec, 0>(n);
        test_matrix_control<Prec, 1>(n);
        test_matrix_control<Prec, 2>(n);
        test_matrix_control<Prec, 3>(n);
    }
}
