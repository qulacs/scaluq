...
template <Precision Prec, Gate<Prec> (*QuantumGateConstructor)()>
void run_random_batched_gate_apply(std::uint64_t n_qubits) {
    ...
    for (int repeat = 0; repeat < 10; repeat++) {
        auto states = StateVectorBatched<Prec>::Haar_random_state(BATCH_SIZE, n_qubits, true);
        auto state_cp = states.get_state_vector_at(0).get_amplitudes();
        for (int i = 0; i < dim; i++) {
            test_state[i] = state_cp[i];
        }

        const Gate<Prec> gate = QuantumGateConstructor();
...
template <Precision Prec,
          Gate<Prec> (*QuantumGateConstructor)(double, const std::vector<std::uint64_t>&)>
void run_random_batched_gate_apply(std::uint64_t n_qubits) {
    const int dim = 1ULL << n_qubits;
    Random random;
    ...
    for (int repeat = 0; repeat < 10; repeat++) {
        ...
        const Gate<Prec> gate = QuantumGateConstructor(angle, {});
        gate->update_quantum_state(states);
...
template <Precision Prec,
          Gate<Prec> (*QuantumGateConstructor)(std::uint64_t, const std::vector<std::uint64_t>&)>
void run_random_batched_gate_apply(std::uint64_t n_qubits,
    ...
    for (int repeat = 0; repeat < 10; repeat++) {
        ...
        const Gate<Prec> gate = QuantumGateConstructor(target, {});
        gate->update_quantum_state(states);
...
template <
    Precision Prec,
    Gate<Prec> (*QuantumGateConstructor)(std::uint64_t, double, const std::vector<std::uint64_t>&)>
void run_random_batched_gate_apply(std::uint64_t n_qubits,
    ...
    for (int repeat = 0; repeat < 10; repeat++) {
        ...
        const std::uint64_t target = random.int64() % n_qubits;
        const Gate<Prec> gate = QuantumGateConstructor(target, angle, {});
        gate->update_quantum_state(states);
...
void run_random_batched_gate_apply_IBMQ(
    ...
    for (int repeat = 0; repeat < 10; repeat++) {
        ...
        for (int gate_type = 0; gate_type < 3; gate_type++) {
            ...
            Gate<Prec> gate;
            if (gate_type == 0) {
                gate = gate::U1<Prec>(target, lambda, {});
            } else if (gate_type == 1) {
                gate = gate::U2<Prec>(target, phi, lambda, {});
            } else {
                gate = gate::U3<Prec>(target, theta, phi, lambda, {});
            }
            gate->update_quantum_state(states);
...
void run_random_batched_gate_apply_two_target(std::uint64_t n_qubits) {
    ...
    for (int repeat = 0; repeat < 10; repeat++) {
        ...
        for (int g = 0; g < 2; g++) {
            ...
            if (g == 0) {
                gate = gate::CX<Prec>(control, target);
                func_eig = get_eigen_matrix_full_qubit_CX;
            } else {
                gate = gate::CZ<Prec>(control, target);
    ...
    for (int repeat = 0; repeat < 10; repeat++) {
        ...
        auto gate = gate::Swap<Prec>(target1, target2);
        gate->update_quantum_state(states);
...
void run_random_batched_gate_apply_pauli(std::uint64_t n_qubits) {
    ...
    for (int repeat = 0; repeat < 10; repeat++) {
        ...
        Gate<Prec> pauli_gate = gate::Pauli<Prec>(pauli);
        ...
        Gate<Prec> pauli_gate_inv = pauli_gate->get_inverse();
    ...
    for (int repeat = 0; repeat < 10; repeat++) {
        ...
        PauliOperator<Prec> pauli(target_vec, pauli_id_vec, 1.0);
        Gate<Prec> pauli_gate = gate::PauliRotation(pauli, angle);
        pauli_gate->update_quantum_state(states);
...
void run_random_batched_gate_apply_none_dense(std::uint64_t n_qubits) {
    ...
    for (std::uint64_t rep = 0; rep < max_repeat; rep++) {
        StateVectorBatched<Prec> states =
            StateVectorBatched<Prec>::Haar_random_state(BATCH_SIZE, n_qubits, true);
        auto state_cp = states.get_state_vector_at(0).get_amplitudes();
        for (std::uint64_t i = 0; i < dim; i++) {
            test_state[i] = state_cp[i];
        }
        ...
        Gate<Prec> dense_gate = gate::DenseMatrix<Prec>(target_list, U, control_list);
...
void run_random_batched_gate_apply_single_dense(std::uint64_t n_qubits) {
    ...
    for (std::uint64_t rep = 0; rep < max_repeat; rep++) {
        ...
        std::vector<std::uint64_t> control_list = {};
        Gate<Prec> dense_gate = gate::DenseMatrix<Prec>(target_list, mat, control_list);
        dense_gate->update_quantum_state(states);
...
void run_random_batched_gate_apply_sparse(std::uint64_t n_qubits) {
    ...
    for (std::uint64_t rep = 0; rep < max_repeat; rep++) {
        ...
        Gate<Prec> sparse_gate = gate::SparseMatrix<Prec>(target_list, mat, control_list);
...
void run_random_batched_gate_apply_general_dense(std::uint64_t n_qubits) {
    ...
    // general single
    {
        for (std::uint64_t rep = 0; rep < max_repeat; rep++) {
            ...
            std::vector<std::uint64_t> control_list = {};
            Gate<Prec> dense_gate = gate::DenseMatrix<Prec>(target_list, U1, control_list);
            dense_gate->update_quantum_state(states);
            auto states_cp = states.get_amplitudes();
            for (std::uint64_t batch_id = 0; batch_id < states.batch_size(); batch_id++) {
                for (std::uint64_t i = 0; i < dim; i++) {
    ...
    // general double
    {
        ...
        for (std::uint64_t rep = 0; rep < max_repeat; rep++) {
            ...
            std::vector<std::uint64_t> control_list = {};
            Gate<Prec> dense_gate = gate::DenseMatrix<Prec>(target_list, Umerge, control_list);
            dense_gate->update_quantum_state(states);
            auto states_cp = states.get_amplitudes();
            for (std::uint64_t batch_id = 0; batch_id < states.batch_size(); batch_id++) {
                for (std::uint64_t i = 0; i < dim; i++) {
...
TYPED_TEST(BatchedGateTest, ApplyI) {
    ...
    run_random_batched_gate_apply<Prec, gate::I<Prec>>(5);
}
TYPED_TEST(BatchedGateTest, ApplyGlobalPhase) {
    constexpr Precision Prec = TestFixture::Prec;
    run_random_batched_gate_apply<Prec, gate::GlobalPhase<Prec>>(5);
}
TYPED_TEST(BatchedGateTest, ApplyX) {
    constexpr Precision Prec = TestFixture::Prec;
    run_random_batched_gate_apply<Prec, gate::X<Prec>>(5, make_X);
}
TYPED_TEST(BatchedGateTest, ApplyY) {
    constexpr Precision Prec = TestFixture::Prec;
    run_random_batched_gate_apply<Prec, gate::Y<Prec>>(5, make_Y);
}
TYPED_TEST(BatchedGateTest, ApplyZ) {
    constexpr Precision Prec = TestFixture::Prec;
    run_random_batched_gate_apply<Prec, gate::Z<Prec>>(5, make_Z);
}
TYPED_TEST(BatchedGateTest, ApplyH) {
    constexpr Precision Prec = TestFixture::Prec;
    run_random_batched_gate_apply<Prec, gate::H<Prec>>(5, make_H);
}
TYPED_TEST(BatchedGateTest, ApplyS) {
    constexpr Precision Prec = TestFixture::Prec;
    run_random_batched_gate_apply<Prec, gate::S<Prec>>(5, make_S);
}
TYPED_TEST(BatchedGateTest, ApplySdag) {
    constexpr Precision Prec = TestFixture::Prec;
    run_random_batched_gate_apply<Prec, gate::Sdag<Prec>>(5, make_Sdag);
}
TYPED_TEST(BatchedGateTest, ApplyT) {
    constexpr Precision Prec = TestFixture::Prec;
    run_random_batched_gate_apply<Prec, gate::T<Prec>>(5, make_T);
}
TYPED_TEST(BatchedGateTest, ApplyTdag) {
    constexpr Precision Prec = TestFixture::Prec;
    run_random_batched_gate_apply<Prec, gate::Tdag<Prec>>(5, make_Tdag);
}
TYPED_TEST(BatchedGateTest, ApplySqrtX) {
    constexpr Precision Prec = TestFixture::Prec;
    run_random_batched_gate_apply<Prec, gate::SqrtX<Prec>>(5, make_SqrtX);
}
TYPED_TEST(BatchedGateTest, ApplySqrtY) {
    constexpr Precision Prec = TestFixture::Prec;
    run_random_batched_gate_apply<Prec, gate::SqrtY<Prec>>(5, make_SqrtY);
}
TYPED_TEST(BatchedGateTest, ApplySqrtXdag) {
    constexpr Precision Prec = TestFixture::Prec;
    run_random_batched_gate_apply<Prec, gate::SqrtXdag<Prec>>(5, make_SqrtXdag);
}
TYPED_TEST(BatchedGateTest, ApplySqrtYdag) {
    constexpr Precision Prec = TestFixture::Prec;
    run_random_batched_gate_apply<Prec, gate::SqrtYdag<Prec>>(5, make_SqrtYdag);
}
TYPED_TEST(BatchedGateTest, ApplyP0) {
    constexpr Precision Prec = TestFixture::Prec;
    run_random_batched_gate_apply<Prec, gate::P0<Prec>>(5, make_P0);
}
TYPED_TEST(BatchedGateTest, ApplyP1) {
    constexpr Precision Prec = TestFixture::Prec;
    run_random_batched_gate_apply<Prec, gate::P1<Prec>>(5, make_P1);
}
TYPED_TEST(BatchedGateTest, ApplyRX) {
    constexpr Precision Prec = TestFixture::Prec;
    run_random_batched_gate_apply<Prec, gate::RX<Prec>>(5, make_RX);
}
TYPED_TEST(BatchedGateTest, ApplyRY) {
    constexpr Precision Prec = TestFixture::Prec;
    run_random_batched_gate_apply<Prec, gate::RY<Prec>>(5, make_RY);
}
TYPED_TEST(BatchedGateTest, ApplyRZ) {
    constexpr Precision Prec = TestFixture::Prec;
    run_random_batched_gate_apply<Prec, gate::RZ<Prec>>(5, make_RZ);
}

TYPED_TEST(BatchedGateTest, ApplyIBMQ) {
    constexpr Precision Prec = TestFixture::Prec;
    run_random_batched_gate_apply_IBMQ<Prec>(5, make_U);
}
TYPED_TEST(BatchedGateTest, ApplySparseMatrixGate) {
    constexpr Precision Prec = TestFixture::Prec;
    run_random_batched_gate_apply_sparse<Prec>(6);
}
TYPED_TEST(BatchedGateTest, ApplyDenseMatrixGate) {
    constexpr Precision Prec = TestFixture::Prec;
    run_random_batched_gate_apply_none_dense<Prec>(6);
    run_random_batched_gate_apply_single_dense<Prec>(6);
    run_random_batched_gate_apply_general_dense<Prec>(6);
}
TYPED_TEST(BatchedGateTest, ApplyPauliGate) {
    constexpr Precision Prec = TestFixture::Prec;
    run_random_batched_gate_apply_pauli<Prec>(5);
}
...
TYPED_TEST(BatchedGateTest, ApplyProbablisticGate) {
    constexpr Precision Prec = TestFixture::Prec;
    {
        ...
        StateVectorBatched<Prec> states(BATCH_SIZE, 1);
...
void test_batched_gate(Gate<Prec> gate_control,
    ...
    StateVectorBatched<Prec> states_controlled(BATCH_SIZE, n_qubits - std::popcount(control_mask));
...
template <Precision Prec, std::uint64_t num_target, std::uint64_t num_rotation, typename Factory>
void test_batched_standard_gate_control(Factory factory, std::uint64_t n) {
    ...
    if constexpr (num_target == 0 && num_rotation == 1) {
        ...
        Gate<Prec> g1 = factory(targets[0], angles[0], angles[1], angles[2], controls);
        Gate<Prec> g2 =
            factory(targets[0] - std::popcount(control_mask & ((1ULL << targets[0]) - 1)),
                    angles[0],
                    angles[1],
        ...
        Gate<Prec> g1 = factory(targets[0], targets[1], controls);
        Gate<Prec> g2 =
            factory(targets[0] - std::popcount(control_mask & ((1ULL << targets[0]) - 1)),
...
template <Precision Prec, bool rotation>
void test_batched_pauli_control(std::uint64_t n) {
    typename PauliOperator<Prec>::Data data1, data2;
    ...
    if constexpr (!rotation) {
        Gate<Prec> g1 = gate::Pauli<Prec>(PauliOperator<Prec>(data1), controls);
        Gate<Prec> g2 = gate::Pauli<Prec>(PauliOperator<Prec>(data2), {});
        test_batched_gate(g1, g2, n, control_mask);
    } else {
        double angle = random.uniform() * std::numbers::pi * 2;
        Gate<Prec> g1 = gate::PauliRotation<Prec>(PauliOperator<Prec>(data1), angle, controls);
        Gate<Prec> g2 = gate::PauliRotation<Prec>(PauliOperator<Prec>(data2), angle, {});
        test_batched_gate(g1, g2, n, control_mask);
    }
}

template <Precision Prec, std::uint64_t num_target>
void test_batched_matrix_control(std::uint64_t n_qubits) {
    ...
    if constexpr (num_target == 0) {
        ...
        mat(0, 0) = val / norm;
        Gate<Prec> d1 = gate::DenseMatrix<Prec>(targets, mat, controls);
        Gate<Prec> d2 = gate::DenseMatrix<Prec>(new_targets, mat, {});
        Gate<Prec> s1 = gate::SparseMatrix<Prec>(targets, mat.sparseView(), controls);
        Gate<Prec> s2 = gate::SparseMatrix<Prec>(new_targets, mat.sparseView(), {});
...
TYPED_TEST(BatchedGateTest, Control) {
    ...
    for ([[maybe_unused]] std::uint64_t _ : std::views::iota(0, 10)) {
        test_batched_standard_gate_control<Prec, 0, 1>(gate::GlobalPhase<Prec>, n);
        test_batched_standard_gate_control<Prec, 1, 0>(gate::X<Prec>, n);
        test_batched_standard_gate_control<Prec, 1, 0>(gate::Y<Prec>, n);
        test_batched_standard_gate_control<Prec, 1, 0>(gate::Z<Prec>, n);
        test_batched_standard_gate_control<Prec, 1, 0>(gate::S<Prec>, n);
        test_batched_standard_gate_control<Prec, 1, 0>(gate::Sdag<Prec>, n);
        test_batched_standard_gate_control<Prec, 1, 0>(gate::T<Prec>, n);
        test_batched_standard_gate_control<Prec, 1, 0>(gate::Tdag<Prec>, n);
        test_batched_standard_gate_control<Prec, 1, 0>(gate::SqrtX<Prec>, n);
        test_batched_standard_gate_control<Prec, 1, 0>(gate::SqrtXdag<Prec>, n);
        test_batched_standard_gate_control<Prec, 1, 0>(gate::SqrtY<Prec>, n);
        test_batched_standard_gate_control<Prec, 1, 0>(gate::SqrtYdag<Prec>, n);
        test_batched_standard_gate_control<Prec, 1, 0>(gate::P0<Prec>, n);
        test_batched_standard_gate_control<Prec, 1, 0>(gate::P1<Prec>, n);
        test_batched_standard_gate_control<Prec, 1, 1>(gate::RX<Prec>, n);
        test_batched_standard_gate_control<Prec, 1, 1>(gate::RY<Prec>, n);
        test_batched_standard_gate_control<Prec, 1, 1>(gate::RZ<Prec>, n);
        test_batched_standard_gate_control<Prec, 1, 1>(gate::U1<Prec>, n);
        test_batched_standard_gate_control<Prec, 1, 2>(gate::U2<Prec>, n);
        test_batched_standard_gate_control<Prec, 1, 3>(gate::U3<Prec>, n);
        test_batched_standard_gate_control<Prec, 2, 0>(gate::Swap<Prec>, n);
        test_batched_pauli_control<Prec, false>(n);
        test_batched_pauli_control<Prec, true>(n);
        test_batched_matrix_control<Prec, 0>(n);
        test_batched_matrix_control<Prec, 1>(n);
        test_batched_matrix_control<Prec, 2>(n);
        test_batched_matrix_control<Prec, 3>(n);
    }
}
