...
template <Precision Prec, ExecutionSpace Space, Gate<Prec> (*QuantumGateConstructor)()>
...
template <Precision Prec,
          ...
          Gate<Prec> (*QuantumGateConstructor)(
...
void run_random_batched_gate_apply_none_dense(std::uint64_t n_qubits) {
    ...
    for (std::uint64_t rep = 0; rep < max_repeat; rep++) {
        ...
        Gate<Prec> dense_gate = gate::DenseMatrix<Prec, Space>(target_list, U, control_list);
...
void run_random_batched_gate_apply_single_dense(std::uint64_t n_qubits) {
    ...
    for (std::uint64_t rep = 0; rep < max_repeat; rep++) {
        ...
        std::vector<std::uint64_t> control_list = {};
        Gate<Prec> dense_gate = gate::DenseMatrix<Prec, Space>(target_list, mat, control_list);
        dense_gate->update_quantum_state(states);
...
void run_random_batched_gate_apply_general_dense(std::uint64_t n_qubits) {
    ...
    // general single
    {
        for (std::uint64_t rep = 0; rep < max_repeat; rep++) {
            ...
            std::vector<std::uint64_t> control_list = {};
            Gate<Prec> dense_gate = gate::DenseMatrix<Prec, Space>(target_list, U1, control_list);
            dense_gate->update_quantum_state(states);
            auto states_cp = states.get_amplitudes();
            for (std::uint64_t batch_id = 0; batch_id < states.batch_size(); batch_id++) {
                for (std::uint64_t i = 0; i < dim; i++) {
...
TYPED_TEST(BatchedGateTest, ApplyI) {
    ...
    run_random_batched_gate_apply<Prec, Space, gate::I<Prec>>(5);
}
TYPED_TEST(BatchedGateTest, ApplyGlobalPhase) {
    constexpr Precision Prec = TestFixture::Prec;
    constexpr ExecutionSpace Space = TestFixture::Space;
    run_random_batched_gate_apply<Prec, Space, gate::GlobalPhase<Prec>>(5);
}
TYPED_TEST(BatchedGateTest, ApplyX) {
    constexpr Precision Prec = TestFixture::Prec;
    constexpr ExecutionSpace Space = TestFixture::Space;
    run_random_batched_gate_apply<Prec, Space, gate::X<Prec>>(5, make_X);
}
TYPED_TEST(BatchedGateTest, ApplyY) {
    constexpr Precision Prec = TestFixture::Prec;
    constexpr ExecutionSpace Space = TestFixture::Space;
    run_random_batched_gate_apply<Prec, Space, gate::Y<Prec>>(5, make_Y);
}
TYPED_TEST(BatchedGateTest, ApplyZ) {
    constexpr Precision Prec = TestFixture::Prec;
    constexpr ExecutionSpace Space = TestFixture::Space;
    run_random_batched_gate_apply<Prec, Space, gate::Z<Prec>>(5, make_Z);
}
TYPED_TEST(BatchedGateTest, ApplyH) {
    constexpr Precision Prec = TestFixture::Prec;
    constexpr ExecutionSpace Space = TestFixture::Space;
    run_random_batched_gate_apply<Prec, Space, gate::H<Prec>>(5, make_H);
}
TYPED_TEST(BatchedGateTest, ApplyS) {
    constexpr Precision Prec = TestFixture::Prec;
    constexpr ExecutionSpace Space = TestFixture::Space;
    run_random_batched_gate_apply<Prec, Space, gate::S<Prec>>(5, make_S);
}
TYPED_TEST(BatchedGateTest, ApplySdag) {
    constexpr Precision Prec = TestFixture::Prec;
    constexpr ExecutionSpace Space = TestFixture::Space;
    run_random_batched_gate_apply<Prec, Space, gate::Sdag<Prec>>(5, make_Sdag);
}
TYPED_TEST(BatchedGateTest, ApplyT) {
    constexpr Precision Prec = TestFixture::Prec;
    constexpr ExecutionSpace Space = TestFixture::Space;
    run_random_batched_gate_apply<Prec, Space, gate::T<Prec>>(5, make_T);
}
TYPED_TEST(BatchedGateTest, ApplyTdag) {
    constexpr Precision Prec = TestFixture::Prec;
    constexpr ExecutionSpace Space = TestFixture::Space;
    run_random_batched_gate_apply<Prec, Space, gate::Tdag<Prec>>(5, make_Tdag);
}
TYPED_TEST(BatchedGateTest, ApplySqrtX) {
    constexpr Precision Prec = TestFixture::Prec;
    constexpr ExecutionSpace Space = TestFixture::Space;
    run_random_batched_gate_apply<Prec, Space, gate::SqrtX<Prec>>(5, make_SqrtX);
}
TYPED_TEST(BatchedGateTest, ApplySqrtY) {
    constexpr Precision Prec = TestFixture::Prec;
    constexpr ExecutionSpace Space = TestFixture::Space;
    run_random_batched_gate_apply<Prec, Space, gate::SqrtY<Prec>>(5, make_SqrtY);
}
TYPED_TEST(BatchedGateTest, ApplySqrtXdag) {
    constexpr Precision Prec = TestFixture::Prec;
    constexpr ExecutionSpace Space = TestFixture::Space;
    run_random_batched_gate_apply<Prec, Space, gate::SqrtXdag<Prec>>(5, make_SqrtXdag);
}
TYPED_TEST(BatchedGateTest, ApplySqrtYdag) {
    constexpr Precision Prec = TestFixture::Prec;
    constexpr ExecutionSpace Space = TestFixture::Space;
    run_random_batched_gate_apply<Prec, Space, gate::SqrtYdag<Prec>>(5, make_SqrtYdag);
}
TYPED_TEST(BatchedGateTest, ApplyP0) {
    constexpr Precision Prec = TestFixture::Prec;
    constexpr ExecutionSpace Space = TestFixture::Space;
    run_random_batched_gate_apply<Prec, Space, gate::P0<Prec>>(5, make_P0);
}
TYPED_TEST(BatchedGateTest, ApplyP1) {
    constexpr Precision Prec = TestFixture::Prec;
    constexpr ExecutionSpace Space = TestFixture::Space;
    run_random_batched_gate_apply<Prec, Space, gate::P1<Prec>>(5, make_P1);
}
TYPED_TEST(BatchedGateTest, ApplyRX) {
    constexpr Precision Prec = TestFixture::Prec;
    constexpr ExecutionSpace Space = TestFixture::Space;
    run_random_batched_gate_apply<Prec, Space, gate::RX<Prec>>(5, make_RX);
}
TYPED_TEST(BatchedGateTest, ApplyRY) {
    constexpr Precision Prec = TestFixture::Prec;
    constexpr ExecutionSpace Space = TestFixture::Space;
    run_random_batched_gate_apply<Prec, Space, gate::RY<Prec>>(5, make_RY);
}
TYPED_TEST(BatchedGateTest, ApplyRZ) {
    constexpr Precision Prec = TestFixture::Prec;
    constexpr ExecutionSpace Space = TestFixture::Space;
    run_random_batched_gate_apply<Prec, Space, gate::RZ<Prec>>(5, make_RZ);
}
    ...
    for (std::size_t i = 0; i < BATCH_SIZE; i++) {
        for (std::uint64_t j = 0; j < states_controlled.dim(); j++) {
            amplitudes_controlled[i][j] =
                amplitudes[i][scaluq::internal::insert_zero_at_mask_positions(j, control_mask) |
                              control_value_mask];
        }
    }
...
void test_batched_standard_gate_control(Factory factory, std::uint64_t n) {
    ...
    if constexpr (num_target == 0 && num_rotation == 1) {
        Gate<Prec> g1 = factory(angles[0], controls, control_values);
        Gate<Prec> g2 = factory(angles[0], {}, {});
        test_batched_gate<Prec, Space>(g1, g2, n, control_mask, control_value_mask);
    } else if constexpr (num_target == 1 && num_rotation == 0) {
        Gate<Prec> g1 = factory(targets[0], controls, control_values);
        Gate<Prec> g2 =
            factory(targets[0] - std::popcount(control_mask & ((1ULL << targets[0]) - 1)), {}, {});
        test_batched_gate<Prec, Space>(g1, g2, n, control_mask, control_value_mask);
    } else if constexpr (num_target == 1 && num_rotation == 1) {
        Gate<Prec> g1 = factory(targets[0], angles[0], controls, control_values);
        Gate<Prec> g2 =
            factory(targets[0] - std::popcount(control_mask & ((1ULL << targets[0]) - 1)),
                    angles[0],
                    {},
        ...
        Gate<Prec> g1 = factory(targets[0], angles[0], angles[1], controls, control_values);
        Gate<Prec> g2 =
            factory(targets[0] - std::popcount(control_mask & ((1ULL << targets[0]) - 1)),
                    angles[0],
                    angles[1],
        ...
        Gate<Prec> g1 = factory(targets[0], targets[1], controls, control_values);
        Gate<Prec> g2 =
            factory(targets[0] - std::popcount(control_mask & ((1ULL << targets[0]) - 1)),
...
void test_batched_pauli_control(std::uint64_t n) {
    ...
    if constexpr (!rotation) {
        Gate<Prec> g1 = gate::Pauli<Prec>(pauli1, controls, control_values);
        Gate<Prec> g2 = gate::Pauli<Prec>(pauli2, {});
        test_batched_gate<Prec, Space>(g1, g2, n, control_mask, control_value_mask);
    } else {
        double angle = random.uniform() * std::numbers::pi * 2;
        Gate<Prec> g1 = gate::PauliRotation<Prec>(pauli1, angle, controls, control_values);
        Gate<Prec> g2 = gate::PauliRotation<Prec>(pauli2, angle, {});
        test_batched_gate<Prec, Space>(g1, g2, n, control_mask, control_value_mask);
    }
}
...
void test_batched_matrix_control(std::uint64_t n_qubits) {
    ...
    if constexpr (num_target == 0) {
        ...
        mat(0, 0) = val / norm;
        Gate<Prec> d1 = gate::DenseMatrix<Prec, Space>(targets, mat, controls, control_values);
        Gate<Prec> d2 = gate::DenseMatrix<Prec, Space>(new_targets, mat, {});
        Gate<Prec> s1 =
            gate::SparseMatrix<Prec, Space>(targets, mat.sparseView(), controls, control_values);
        Gate<Prec> s2 = gate::SparseMatrix<Prec, Space>(new_targets, mat.sparseView(), {});
        test_batched_gate<Prec, Space>(d1, d2, n_qubits, control_mask, control_value_mask);
        test_batched_gate<Prec, Space>(s1, s2, n_qubits, control_mask, control_value_mask);
...
TYPED_TEST(BatchedGateTest, Control) {
    ...
    for ([[maybe_unused]] std::uint64_t _ : std::views::iota(0, 10)) {
        test_batched_standard_gate_control<Prec, Space, 0, 1>(gate::GlobalPhase<Prec>, n);
        test_batched_standard_gate_control<Prec, Space, 1, 0>(gate::X<Prec>, n);
        test_batched_standard_gate_control<Prec, Space, 1, 0>(gate::Y<Prec>, n);
        test_batched_standard_gate_control<Prec, Space, 1, 0>(gate::Z<Prec>, n);
        test_batched_standard_gate_control<Prec, Space, 1, 0>(gate::S<Prec>, n);
        test_batched_standard_gate_control<Prec, Space, 1, 0>(gate::Sdag<Prec>, n);
        test_batched_standard_gate_control<Prec, Space, 1, 0>(gate::T<Prec>, n);
        test_batched_standard_gate_control<Prec, Space, 1, 0>(gate::Tdag<Prec>, n);
        test_batched_standard_gate_control<Prec, Space, 1, 0>(gate::SqrtX<Prec>, n);
        test_batched_standard_gate_control<Prec, Space, 1, 0>(gate::SqrtXdag<Prec>, n);
        test_batched_standard_gate_control<Prec, Space, 1, 0>(gate::SqrtY<Prec>, n);
        test_batched_standard_gate_control<Prec, Space, 1, 0>(gate::SqrtYdag<Prec>, n);
        test_batched_standard_gate_control<Prec, Space, 1, 0>(gate::P0<Prec>, n);
        test_batched_standard_gate_control<Prec, Space, 1, 0>(gate::P1<Prec>, n);
        test_batched_standard_gate_control<Prec, Space, 1, 1>(gate::RX<Prec>, n);
        test_batched_standard_gate_control<Prec, Space, 1, 1>(gate::RY<Prec>, n);
        test_batched_standard_gate_control<Prec, Space, 1, 1>(gate::RZ<Prec>, n);
        test_batched_standard_gate_control<Prec, Space, 1, 1>(gate::U1<Prec>, n);
