...
void param_circuit_test() {
    ...
    for ([[maybe_unused]] std::uint64_t repeat : std::views::iota(0, 10)) {
        Circuit<Fp> circuit(n_qubits);
        Circuit<Fp> pcircuit(n_qubits);
        ...
        auto add_random_rotation_or_cnot = [&] {
            if (random.int32() & 1) {
                ...
                if (param_gate_kind == 0) {
                    ...
                    circuit.add_gate(gate::PauliRotation<Fp>(pauli, coef * params[pidx]));
                    pcircuit.add_param_gate(gate::ParamPauliRotation<Fp>(pauli, coef), pkeys[pidx]);
                }
            } else {
                std::uint64_t control = random.int32() % n_qubits;
                std::uint64_t target = random.int32() % (n_qubits - 1);
                if (target == control) target = n_qubits - 1;
                circuit.add_gate(gate::CX<Fp>(control, target));
                pcircuit.add_gate(gate::CX<Fp>(control, target));
            }
        };
        ...
        for (std::uint64_t idx : std::views::iota(std::uint64_t{0}, 1ULL << n_qubits)) {
            check_near((StdComplex<Fp>)amplitudes[idx], (StdComplex<Fp>)amplitudes_cp[idx]);
        }
        ...
        for (std::uint64_t idx : std::views::iota(std::uint64_t{0}, 1ULL << n_qubits)) {
            check_near((StdComplex<Fp>)amplitudes[idx], (StdComplex<Fp>)amplitudes_base[idx]);
            check_near((StdComplex<Fp>)amplitudes_cp[idx], (StdComplex<Fp>)amplitudes_base[idx]);
        }
    }
}

TEST(ParamCircuitTest, ApplyParamCircuit) {
    param_circuit_test<double>();
    param_circuit_test<float>();
}

TEST(ParamCircuitTest, InsufficientParameterGiven) {
    {
        Circuit<double> circuit(1);
        circuit.add_param_gate(gate::ParamRX<double>(0), "0");
        circuit.add_param_gate(gate::ParamRX<double>(0), "1");
        circuit.add_param_gate(gate::ParamRX<double>(0), "0");
    ...
    {
        Circuit<float> circuit(1);
        circuit.add_param_gate(gate::ParamRX<float>(0), "0");
        circuit.add_param_gate(gate::ParamRX<float>(0), "1");
        circuit.add_param_gate(gate::ParamRX<float>(0), "0");
