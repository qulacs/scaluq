...
Eigen::MatrixXcd kronecker_product(const Eigen::MatrixXcd& lhs, const Eigen::MatrixXcd& rhs) {
...
Eigen::MatrixXcd get_eigen_matrix_single_Pauli(UINT pauli_id) {
...
Eigen::MatrixXcd get_eigen_matrix_random_single_qubit_unitary() {
...
Eigen::VectorXcd get_eigen_diagonal_matrix_random_multi_qubit_unitary(UINT qubit_count) {
...
Eigen::MatrixXcd get_expanded_eigen_matrix_with_identity(UINT target_qubit_index,
                                                         const Eigen::MatrixXcd& one_qubit_matrix,
                                                         UINT qubit_count) {
...
// get expanded matrix
Eigen::MatrixXcd get_eigen_matrix_full_qubit_pauli(std::vector<UINT> pauli_ids) {
...
Eigen::MatrixXcd get_eigen_matrix_full_qubit_pauli(std::vector<UINT> index_list,
...
Eigen::MatrixXcd get_eigen_matrix_full_qubit_CNOT(UINT control_qubit_index,
...
Eigen::MatrixXcd get_eigen_matrix_full_qubit_CZ(UINT control_qubit_index,
                                                UINT target_qubit_index,
                                                UINT qubit_count) {
    UINT dim = 1ULL << qubit_count;
    Eigen::MatrixXcd result = Eigen::MatrixXcd::Zero(dim, dim);
...
Eigen::MatrixXcd get_eigen_matrix_full_qubit_SWAP(UINT target_qubit_index1,
...
Eigen::MatrixXcd make_2x2_matrix(const Eigen::dcomplex a00,
...
Eigen::MatrixXcd make_I() { return Eigen::MatrixXcd::Identity(2, 2); }

Eigen::MatrixXcd make_X() { return make_2x2_matrix(0, 1, 1, 0); }

Eigen::MatrixXcd make_Y() { return make_2x2_matrix(0, -1.i, 1.i, 0); }

Eigen::MatrixXcd make_Z() { return make_2x2_matrix(1, 0, 0, -1); }

Eigen::MatrixXcd make_H() {
    return make_2x2_matrix(1 / sqrt(2.), 1 / sqrt(2.), 1 / sqrt(2.), -1 / sqrt(2.));
}

Eigen::MatrixXcd make_S() { return make_2x2_matrix(1, 0, 0, 1.i); }

Eigen::MatrixXcd make_Sdag() { return make_2x2_matrix(1, 0, 0, -1.i); }

Eigen::MatrixXcd make_T() { return make_2x2_matrix(1, 0, 0, (1. + 1.i) / sqrt(2.)); }

Eigen::MatrixXcd make_Tdag() { return make_2x2_matrix(1, 0, 0, (1. - 1.i) / sqrt(2.)); }

Eigen::MatrixXcd make_sqrtX() {
    return make_2x2_matrix(0.5 + 0.5i, 0.5 - 0.5i, 0.5 - 0.5i, 0.5 + 0.5i);
}

Eigen::MatrixXcd make_sqrtY() {
    return make_2x2_matrix(0.5 + 0.5i, -0.5 - 0.5i, 0.5 + 0.5i, 0.5 + 0.5i);
}

Eigen::MatrixXcd make_sqrtXdag() {
    return make_2x2_matrix(0.5 - 0.5i, 0.5 + 0.5i, 0.5 + 0.5i, 0.5 - 0.5i);
}

Eigen::MatrixXcd make_sqrtYdag() {
    return make_2x2_matrix(0.5 - 0.5i, 0.5 - 0.5i, -0.5 + 0.5i, 0.5 - 0.5i);
}

Eigen::MatrixXcd make_P0() { return make_2x2_matrix(1, 0, 0, 0); }

Eigen::MatrixXcd make_P1() { return make_2x2_matrix(0, 0, 0, 1); }

Eigen::MatrixXcd make_RX(double angle) {
